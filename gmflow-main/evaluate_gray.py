...
def inference_on_dir(model,
    ...
    mode_color = 'gray' #'gray'
    small_len = len(os.listdir(base_dir+'rgblr')) 
    disp_l_small = []
    disp_r_small = []
    range_l_small = 0
    range_r_small = 0
    ...
    for test_id in range(0, len(filenames) , stride):
        print(test_id)
        
                
                #print(l_gt.shape,np.mean(l_gt))
                #exit(1)
        '''
        if (test_id)%small_len == 0:
            disp_l_small = []
            disp_r_small = []
            mean_l_small = 0
            mean_r_small = 0
        ...
        if pred_bidir_flow:
            ...
            if fwd_bwd_consistency_check:
                ...
                disp_l = fwd_flow[:,0,:,:].squeeze()            
                disp_r = bwd_flow[:,0,:,:].squeeze()
                v_disp_l = torch.abs(fwd_flow[:,1,:,:].squeeze())
                v_disp_r = torch.abs(bwd_flow[:,1,:,:].squeeze())
                ver_l_small = ver_l_small + (len(v_disp_l[v_disp_l>2])/(v_disp_l.shape[0]*v_disp_l.shape[1]))
                ver_r_small = ver_r_small + (len(v_disp_r[v_disp_r>2])/(v_disp_r.shape[0]*v_disp_r.shape[1]))
                #print(torch.min(v_disp_l),torch.min(v_disp_l),torch.max(v_disp_l))
                #print(torch.min(v_disp_r),torch.min(v_disp_r),torch.max(v_disp_r))
                if mode_video == 'rl':
                    disp_l = - disp_l
                elif mode_video == 'lr':
                    disp_r = - disp_r
                print(torch.min(disp_l),torch.mean(disp_l),torch.max(disp_l))
                range_l_small = range_l_small + (torch.max(disp_l) - torch.min(disp_l)).item()
                range_r_small = range_r_small + (torch.max(disp_r) - torch.min(disp_r)).item()
                disp_l = disp_l.to(torch.device('cpu'))
                disp_r = disp_r.to(torch.device('cpu'))
                disp_l_small.append(disp_l)
                disp_r_small.append(disp_r)
    ...
    disp_l_small = [item.cpu().detach().numpy() for item in disp_l_small]  #torch.Tensor(disp_l_small)
    disp_r_small = [item.cpu().detach().numpy() for item in disp_r_small]  #torch.Tensor(disp_r_small)
    ...
    shift_all_l = []
    shift_all_r = []
    lmin = np.min(disp_l_small)
    lmax = np.max(disp_l_small)
    rmin = np.min(disp_r_small)
    rmax = np.max(disp_r_small)
    '''
    with open(base_dir+'/range.txt','a') as f:
        f.write(str(lmax - lmin)+'\n')
        f.write(str(rmax - rmin)+'\n')
    print('disparity range:',lmax - lmin,rmax - rmin)
    '''
            

    
    if mode_color == 'color':
        for small_id in range(len(disp_l_small)):
            print('saving color:',small_id,'/',len(disp_l_small))
            l_gt = disp_l_small[small_id] #(disp_l_small[small_id]-mean_l_small.item())/std_l_small.item()
            r_gt = disp_r_small[small_id] #(disp_r_small[small_id]-mean_r_small.item())/std_r_small.item()
            mask_sky_l = cv2.imread(base_dir + '/left_sky/'+ 'frame_'+'%06d'%(small_id)+'.png',-1)
            mask_sky_r = cv2.imread(base_dir + '/right_sky/' + 'frame_'+'%06d'%(small_id)+'.png',-1)
            l_gt[mask_sky_l==255] = np.min(l_gt)
            r_gt[mask_sky_r==255] = np.min(r_gt)
            
            mask_con_l = cv2.imread(base_dir + '/flow/' + 'frame_'+'%06d'%(2*(small_id))+'_occ.png',-1)
            mask_con_r = cv2.imread(base_dir + '/flow/' + 'frame_'+'%06d'%(2*(small_id))+'_occ_bwd.png',-1)
            l_gt[mask_con_l==255] = np.min(l_gt)
            r_gt[mask_con_r==255] = np.min(r_gt)
            dir_l = base_dir + '/l_gt/' + 'frame_'+'%06d'%small_id+'.png'
            dir_r = base_dir + '/r_gt/' + 'frame_'+'%06d'%small_id+'.png'
            plt.imsave(dir_l,l_gt, cmap='inferno',vmin=lmin, vmax=lmax)
            plt.imsave(dir_r,r_gt, cmap='inferno',vmin=rmin, vmax=rmax) 
 
    elif mode_color == 'gray':
        for small_id in range(len(disp_l_small)):
        
            l_gt = disp_l_small[small_id] #(disp_l_small[small_id]-mean_l_small.item())/std_l_small.item()
            r_gt = disp_r_small[small_id] #(disp_r_small[small_id]-mean_r_small.item())/std_r_small.item()
            
            l_gt = 65535*((l_gt-lmin)/(lmax-lmin))
            r_gt = 65535*((r_gt-rmin)/(rmax-rmin))

            shift_i_l = np.median(l_gt)
            shift_i_r = np.median(r_gt)
            scale_i_l = np.mean(np.abs(l_gt-shift_i_l))
            scale_i_r = np.mean(np.abs(r_gt-shift_i_r))
            shift_all_l.append(shift_i_l)
            shift_all_r.append(shift_i_r)
            scale_all_l.append(scale_i_l)
            scale_all_r.append(scale_i_r)
            ...
            mask_sky_l = cv2.imread(base_dir + '/left_sky/'+ 'frame_'+'%06d'%(small_id)+'.png',-1)
            mask_sky_r = cv2.imread(base_dir + '/right_sky/' + 'frame_'+'%06d'%(small_id)+'.png',-1)
            l_gt[mask_sky_l==255] = 0 #np.min(l_gt)
            r_gt[mask_sky_r==255] = 0 #np.min(r_gt)
 
            
            #mask_con_l = cv2.imread(base_dir + '/flow/' + 'frame_'+'%06d'%(2*(small_id))+'_occ.png',-1)
            #mask_con_r = cv2.imread(base_dir + '/flow/' + 'frame_'+'%06d'%(2*(small_id))+'_occ_bwd.png',-1)
            #l_gt[mask_con_l==255] = 0 #np.min(l_gt)
            #r_gt[mask_con_r==255] = 0 #np.min(r_gt)
  
            
            dir_l = base_dir + '/left_gt/' + 'frame_'+'%06d'%small_id+'.png'
            dir_r = base_dir + '/right_gt/' + 'frame_'+'%06d'%small_id+'.png'
            
            print('saving gt:',small_id,'/',len(disp_l_small),shift_i_l,shift_i_r,scale_i_l,scale_i_r)
            #print('saving gt:',small_id,'/',len(disp_l_small),np.min(r_gt),np.median(l_gt),np.max(l_gt))
            cv2.imwrite(dir_l, l_gt.astype("uint16"))
            cv2.imwrite(dir_r, r_gt.astype("uint16"))
            #plt.imsave(dir_l,l_gt, cmap='inferno',vmin=lmin, vmax=lmax)
            #plt.imsave(dir_r,r_gt, cmap='inferno',vmin=rmin, vmax=rmax) 
        scale_ave_l = np.mean(scale_all_l)
        scale_ave_r = np.mean(scale_all_r)
        shift_ave_l = np.mean(shift_all_l)
        shift_ave_r = np.mean(shift_all_r)
    
    
        range_l_small = range_l_small / (len(filenames)/2)
        range_r_small = range_r_small / (len(filenames)/2) 
        with open(base_dir+'/range_avg.txt','a') as f:
            f.write(str(range_l_small)+'\n')
            f.write(str(range_r_small)+'\n')
        print('disparity range:',range_l_small,range_r_small)
        ver_l_small = ver_l_small / (len(filenames)/2)
        ver_r_small = ver_r_small / (len(filenames)/2)
        with open(base_dir+'/ver_ratio.txt','a') as f:
            f.write(str(ver_l_small)+'\n')
            f.write(str(ver_r_small)+'\n')
        print('vertical ratio:',ver_l_small,ver_r_small)
        with open(base_dir+'/shift_scale_lr.txt','a') as f:
            f.write(str(shift_ave_l)+'\n')
            f.write(str(shift_ave_r)+'\n')
            f.write(str(scale_ave_l)+'\n')
            f.write(str(scale_ave_r)+'\n')
        print('average scale snd shift:',shift_ave_l,shift_ave_r,scale_ave_l,scale_ave_r)
        
    elif mode_color == 'repair':
        range_l_small = range_l_small / (len(filenames)/2)
        range_r_small = range_r_small / (len(filenames)/2) 
        with open(base_dir+'/range_avg.txt','a') as f:
            f.write(str(range_l_small)+'\n')
        ...
        with open(base_dir+'/ver_ratio.txt','a') as f:
            f.write(str(ver_l_small)+'\n')
            f.write(str(ver_r_small)+'\n')
        print('disparity range:',ver_l_small,ver_r_small)
